
const { Client, Databases } = require('node-appwrite');

const ENDPOINT = 'https://nyc.cloud.appwrite.io/v1';
const PROJECT_ID = '696048f60006dabb8ae3';
const API_KEY = 'standard_';
const DATABASE_ID = '696049d9000983d28c16';
const COLLECTION_CLIENTES = 'clientes';

const client = new Client()
    .setEndpoint(ENDPOINT)
    .setProject(PROJECT_ID)
    .setKey(API_KEY);

const databases = new Databases(client);

async function addAttributes() {
    try {
        console.log('🛠 Adding missing attributes to clientes collection...');

        // 1. Direccion
        console.log('Adding direccion...');
        await databases.createStringAttribute(DATABASE_ID, COLLECTION_CLIENTES, 'direccion', 200, false);
        console.log('✅ direccion added');

        // Wait a bit to avoid rate limits or race conditions
        await new Promise(r => setTimeout(r, 1000));

        // 2. Ciudad
        console.log('Adding ciudad...');
        await databases.createStringAttribute(DATABASE_ID, COLLECTION_CLIENTES, 'ciudad', 100, false, 'Bogotá');
        console.log('✅ ciudad added');

        await new Promise(r => setTimeout(r, 1000));

        // 3. Frecuencia Preferida
        console.log('Adding frecuenciaPreferida...');
        await databases.createEnumAttribute(
            DATABASE_ID,
            COLLECTION_CLIENTES,
            'frecuenciaPreferida',
            ['unica', 'semanal', 'quincenal', 'mensual'],
            false,
            'unica'
        );
        console.log('✅ frecuenciaPreferida added');

        console.log('🎉 Schema update complete! Please wait a few seconds for indexes to build before writing data.');

    } catch (error) {
        console.error('❌ Error adding attributes:', error.message);
        if (error.response) console.error(JSON.stringify(error.response, null, 2));
    }
}

addAttributes();
